 ...
 * @brief Implementation of BT4 bottom friction source term calculations.
 * @details Concrete subroutine for BT4 bottom friction source terms.
...
void SourceTermBT4::calculate(std::span<double> data) {
  std::for_each(data.begin(), data.end(), [](double &val) { val -= 0.004; });
}

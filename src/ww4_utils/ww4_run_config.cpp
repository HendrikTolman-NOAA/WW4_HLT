 ...
 */
std::optional<HomogeneousDataPoint>
parseHomogeneousString(const std::string_view s) {
  if (s.empty())
    return std::nullopt;
...
void echoHomogeneousData(const std::vector<HomogeneousDataPoint> &processed,
                         ...
                         const EchoOption option, std::ostream &os) {
...
std::optional<RunConfig> loadRunConfig(const std::string_view filename,
  ...
  while (std::getline(file, line)) {
    ...
    if (lineView[first] == '-') {
      const std::string_view valueStr = cleanValue(lineView.substr(first + 1));
      if (!valueStr.empty()) {
        auto dp = parseHomogeneousString(valueStr);
        if (!dp) {
          ww4_std_out::extcde(1, os,
                              "Failed to parse homogeneous data: " +
                                  std::string(valueStr),
                              __FILE__, __LINE__);
        }
        if (lastKey == "water_levels") {
          config.homogeneousWaterLevels.push_back(std::move(*dp));
        } else if (lastKey == "currents") {
          config.homogeneousCurrents.push_back(std::move(*dp));
        } else if (lastKey == "winds") {
          config.homogeneousWinds.push_back(std::move(*dp));
        } else if (lastKey == "ice_concentrations") {
          config.homogeneousIceConcentrations.push_back(std::move(*dp));
        } else if (lastKey == "bottom_depth") {
          config.homogeneousBottomDepth.push_back(std::move(*dp));
        }
      }
      continue;
    }

...
/**
 * @struct WaveTimeData
 * @brief Core data structure for the wave model propagation routine.
 * @details Stores the current model time and the time step.
...
struct WaveTimeData {
  ...
  std::optional<ww4_utils::DateTime> modelTime =
      std::nullopt; ///< Current model time.
};
